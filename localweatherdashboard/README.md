...
* **Defensive Empty States:** The `EmptyState` component is essentially unreachable right now, since the offline mock data is always set in the fallback, even on total errors. I left the component in defensively in case the API response shape changes unexpectedly, as that felt safer than letting it silently fail.
...
* **Day/Night Icon Variants:** The weather icons currently default to their daytime variants (e.g., `clear-day`). Open-Meteo provides an `is_day` boolean that I haven't requested yet. Hooking this up is a straightforward additive change: request the flag, add it to the `WeatherData` type, and update `conditionMeta.ts` to toggle between the `-day` and `-night` SVGs. The Meteocons package makes it easy for us to do so, and they have some really nice looking icons for night time too. 

#### 3. Tech Debt & Polish
* **Preventing the Theme Flashbang:** Dark mode currently applies via a `useEffect` on mount. This causes a single-frame flash of unstyled content where the light theme renders before flipping to dark. To fix this, I would add a blocking inline script to the `<head>` of `index.html` to parse `localStorage` and `matchMedia` before the first paint (similar to how `next-themes` handles it).
* **Accessibility Deep Dive:** I resolved all contrast and structural issues flagged by Lighthouse, and the UI relies heavily on accessible primitives. However, Lighthouse only catches a fraction of WCAG requirements. A true production pass would include manual keyboard-navigation testing and a deep sweep using `axe-core`. Even a manual screen reader check would do wonders for a real production.
...
* **Responsive design edge cases**: I skipped working out responsive design for 4K resolution monitors and above, prioritizing phone, 1080p, and 1440p since these are the resolutions covering the large majority of real usage. The dashboard grid's row sizing use 1fr fractions against a fixed height: 100%. This is proportional to available viewport height rather than actual content height, and it shows up as extra whitespace and slightly odd sized icons and text inside cards, when the viewport is well beyond the resolutions tested. Fix would be switching to content-aware row sizing such as "auto" or capping the grid's max-height instead of letting it stretch to fill an arbitrarily tall viewport.
